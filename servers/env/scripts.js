...
(function(exports) {
	...
	ScriptServer.prototype = Object.create(Environment.Server.prototype);

	// request router
	ScriptServer.prototype.handleHttpRequest = function(request, response) {
	...
	// GET|HEAD /
	ScriptServer.prototype.$listServers = function(request, response) {
	...
	// GET|HEAD /:domain
	ScriptServer.prototype.$getServer = function(request, response, match) {
		var respond = Link.responder(response);
	...
	// PUT /:domain
	ScriptServer.prototype.$putServer = function(request, response, match) {
		var respond = Link.responder(response);
		var domain = match.path[1];
		var router = Link.router(request);
		// headers
	...
	// GET /:domain/editor
	ScriptServer.prototype.$getServerEditor = function(request, response, match) {
		var self = this;
		var respond = Link.responder(response);
		var domain = match.path[1];
		// headers
	...
	ScriptServer.prototype.renderServerEditorHtml = function(domain, source) {
	...
	// POST /:domain/editor
	ScriptServer.prototype.$postServerEditor = function(request, response, match) {
		var respond = Link.responder(response);
		var domain = match.path[1];
		var self = this;
		// headers
	...
	exports.ScriptServer = ScriptServer;
})(Grim);